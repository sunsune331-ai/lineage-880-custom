...
public class WeekQuestLoader {
	...
	private void update() {
		...
		try {
			...
			pstm = con.prepareStatement(
					"insert into tb_weekquest_matrix set difficulty=?, col1=?, col2=?, col3=?, stamp=? on duplicate key update col1=?, col2=?, col3=?, stamp=?");
			int idx = 0;
			for (int i = 0; i < 3; i++) {
				idx = 0;
				pstm.setInt(++idx, i);
	...
	private boolean checkDuplicateBookId(MonsterBook[][] matrix, MonsterBook book, int difficulty, int section)
			throws Exception {
