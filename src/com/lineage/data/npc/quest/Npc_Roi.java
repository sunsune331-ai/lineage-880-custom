	...
	public void action(L1PcInstance pc, L1NpcInstance npc, String cmd, long amount) {
		...
		if (pc.getLevel() >= EWLv40_1.QUEST.get_questlevel()) {
			if (cmd.equalsIgnoreCase("start")) {
				L1Npc l1npc = NpcTable.get().getTemplate(70957);
		...
		try {
			...
			for (L1Object object : World.get().getVisibleObjects(npc))
				if ((object instanceof L1NpcInstance)) {
					...
					if (tgnpc.getNpcTemplate().get_npcId() == 70964) {
						if (npc.getLocation().getTileLineDistance(pc.getLocation()) < 3) {
							if (tgnpc.getLocation().getTileLineDistance(pc.getLocation()) < 3) {
								...
								CreateNewItem.getQuestItem(pc, npc, 41003, 1L);
 ...
 * com.lineage.data.npc.quest.Npc_Roi JD-Core Version: 0.6.2
 */