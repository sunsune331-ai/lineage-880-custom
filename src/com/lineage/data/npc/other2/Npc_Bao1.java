...
public class Npc_Bao1 extends NpcExecutor {
	...
	public void talk(final L1PcInstance pc, final L1NpcInstance npc) {
		try {
			pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "bao_map1"));
	...
	public void action(final L1PcInstance pc, final L1NpcInstance npc,
		...
		try {
			...
			if (cmd.equalsIgnoreCase("bao_quest1")) {
				...
				if (pc.getMeteLevel() < ConfigQuest.Bao1_Quest_Levelmet) {
					pc.sendPackets(new S_SystemMessage("您未達"
							+ ConfigQuest.Bao1_Quest_Levelmet + "轉，無法接取任務。"));
					return;
				}

				if (pc.getLevel()
						+ ((pc.getMeteLevel() - ConfigQuest.Bao1_Quest_Levelmet) * 99) < ConfigQuest.Bao1_Quest_Lv) {
					pc.sendPackets(new S_SystemMessage("您未達"
							+ ConfigQuest.Bao1_Quest_Lv + "等，無法接取任務。"));
					return;
				}

				pc.getQuest().set_step(L1PcQuest.BAO_QUEST_1,
