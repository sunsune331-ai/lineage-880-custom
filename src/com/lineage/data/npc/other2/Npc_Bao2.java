			...
			if (cmd.equalsIgnoreCase("bao_quest2")) {
				...
				if (pc.getMeteLevel() < ConfigQuest.Bao2_Quest_Levelmet) {
					pc.sendPackets(new S_SystemMessage("您未達"
							+ ConfigQuest.Bao2_Quest_Levelmet + "轉，無法接取任務。"));
					return;
				}

				if (pc.getLevel()
						+ ((pc.getMeteLevel() - ConfigQuest.Bao2_Quest_Levelmet) * 99) < ConfigQuest.Bao2_Quest_Lv) {
					pc.sendPackets(new S_SystemMessage("您未達"
							+ ConfigQuest.Bao2_Quest_Lv + "等，無法接取任務。"));
					return;
				}

				pc.getQuest().set_step(L1PcQuest.BAO_QUEST_2,
