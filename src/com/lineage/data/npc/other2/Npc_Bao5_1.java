 ...
 * 
 */
public class Npc_Bao5_1 extends NpcExecutor {

	private static final Log _log = LogFactory.getLog(Npc_Bao5_1.class);
	...
	public void talk(final L1PcInstance pc, final L1NpcInstance npc) {
		try {
			pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "bao_map5_1"));
	...
	public void action(final L1PcInstance pc, final L1NpcInstance npc,
		...
		try {
			...
			if (cmd.equalsIgnoreCase("bao_gift5")) {
				...
				if (ConfigQuest.BAO5_QUEST_ITEM != null) {

					boolean error = false;

					final int size = ConfigQuest.BAO5_QUEST_ITEM.length;

					for (int i = 0; i < size; i++) {

						if (!pc.getInventory().checkItem(
								ConfigQuest.BAO5_QUEST_ITEM[i][0],
								ConfigQuest.BAO5_QUEST_ITEM[i][1])) {
							...
							pc.sendPackets(new S_ServerMessage(337, temp
									.getName()
									+ "("
									+ (ConfigQuest.BAO5_QUEST_ITEM[i][1] - pc
					...
					if (error) {
						...
						for (int j = 0; j < size; j++) {
							pc.getInventory().consumeItem(
									ConfigQuest.BAO5_QUEST_ITEM[j][0],
									ConfigQuest.BAO5_QUEST_ITEM[j][1]);
						}

						pc.getQuest().set_end(L1PcQuest.BAO_QUEST_5);
