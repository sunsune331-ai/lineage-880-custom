...
public class Npc_DragonA3 extends NpcExecutor {
	...
	// 龍門OBJID的參加者列表
	private Map<Integer, ArrayList<Integer>> _DOORPLAYERMAP = new HashMap<Integer, ArrayList<Integer>>();
	...
	private void staraQuestA(L1NpcInstance npc, L1PcInstance pc) {
		try {
			...
			QuestClass.get().startQuest(pc, ADLv80_3.QUEST.get_id());// 設定開始任務

			QuestClass.get().endQuest(pc, ADLv80_3.QUEST.get_id());// 設定結束任務

			L1QuestUser quest = WorldQuest.get().put(pc.get_showId(), 1017, questid, pc);// 加入副本清單

			L1Teleport.teleport(pc, 32673, 32925, (short) 1017, 5, true);// 傳送至副本地圖
	...
	private boolean isError2(L1PcInstance pc, L1NpcInstance npc) {
		...
		int users = QuestMapTable.get().getTemplate(1017);// 地圖人數限制
		if (users == -1) {
			users = 127;
		}
		ArrayList<Integer> playerlist = _DOORPLAYERMAP.get(npc.getId());// 此龍門的參加者列表
