...
import com.lineage.data.quest.ADLv80_4;
 ...
 * 巴拉卡斯副本
 */
public class Npc_DragonA4 extends NpcExecutor {

	private static final Log _log = LogFactory.getLog(Npc_DragonA4.class);

	public static NpcExecutor get() {
		return new Npc_DragonA4();
	}

	// 龍門OBJID的參加者列表
	private final Map<Integer, ArrayList<Integer>> _DOORPLAYERMAP = new HashMap<Integer, ArrayList<Integer>>();

	private final ArrayList<Integer> key_runout_doorid = new ArrayList<Integer>();// 已售出鑰匙的龍門OBJID
	...
	public void talk(final L1PcInstance pc, final L1NpcInstance npc) {
		try {
			pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "y_n_dragon1", new String[] { "巴拉卡斯棲息地" }));
		...
		if (cmd.equalsIgnoreCase("1")) {// 購買龍門憑證
			...
			if (isRented) {// 已租用訓練所
				...
				if (pc.getInventory().checkItem(L1ItemId.ADENA, (300 * amount))) {
					final L1Inn inn = InnTable.getInstance().getTemplate(npcId, roomNumber);
					if (inn != null) {
						// 租用時間 2小時
						final Timestamp ts = new Timestamp(System.currentTimeMillis() + (60 * 60 * 2 * 1000));
						// 登入旅館資料
						final L1ItemInstance item = ItemTable.get().createItem(82504); // 龍門憑證
						if (item != null) {
	...
	private void staraQuestA(final L1NpcInstance npc, final L1PcInstance pc) {
		try {
			// 任務編號
			final int questid = ADLv80_4.QUEST.get_id();
			...
			// 將任務設置為執行中
			QuestClass.get().startQuest(pc, ADLv80_4.QUEST.get_id());// 設定開始任務

			// 將任務設置為結束
			QuestClass.get().endQuest(pc, ADLv80_4.QUEST.get_id());// 設定結束任務

			// 加入副本執行成員
			final L1QuestUser quest = WorldQuest.get().put(pc.get_showId(), ADLv80_4.MAPID, questid, pc);// 加入副本清單

			// 傳送任務執行者
			L1Teleport.teleport(pc, 32732, 32930, (short) ADLv80_4.MAPID, 5, true);// 傳送至副本地圖
			...
			// 取回進入時間限制
			final Integer time = QuestMapTable.get().getTime(ADLv80_4.MAPID);
			if (time != null) {
				quest.set_time(time.intValue());
			}

			for (final L1NpcInstance npc2 : quest.npcList()) {// 清空怪物身上道具(BOSS除外)
				if ((npc2 instanceof L1MonsterInstance)) {
					final L1MonsterInstance mob = (L1MonsterInstance) npc2;
					if (mob.getNpcId() >= 230761 && mob.getNpcId() <= 230763) {
						continue;
					}
	...
	private boolean isError2(final L1PcInstance pc, final L1NpcInstance npc) {
		...
		if (pc.hasSkillEffect(DRAGON_BLOOD_4)) {
		...
		// 進入人數限制
		int users = QuestMapTable.get().getTemplate(ADLv80_4.MAPID);// 地圖人數限制
		if (users == -1) { // 無限制
			users = Byte.MAX_VALUE; // 設置為127
		}

		final ArrayList<Integer> playerlist = _DOORPLAYERMAP.get(npc.getId());// 此龍門的參加者列表
