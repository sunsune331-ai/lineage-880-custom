...
public class Dissolution_armor extends ItemExecutor {
	...
	private void useResolvent(final L1PcInstance pc, final L1ItemInstance item ,final L1ItemInstance resolvent) {
		...
		if ((item.getItem().getType2() == 2) || (item.getItem().getType2() == 0)) { //防具/道具
		...
		if (crystalCount != 0) {
			pc.get_other().setArtifact1(pc.get_other().getArtifact1() + crystalCount);//+溶解值
			...
			煉化防具(""
					...
					+ " " + item.getName()
					+ "】- 階級:[" + pc.get_other().getLv_Redmg_Artifact()
	...
	public static void 煉化防具(String info) {
		try {
			BufferedWriter out = new BufferedWriter(new FileWriter("./物品操作日誌/[記錄]-煉化防具.txt", true));
			out.write(info + "\r\n");
			out.close();
		} catch (IOException e) {
			e.printStackTrace();
