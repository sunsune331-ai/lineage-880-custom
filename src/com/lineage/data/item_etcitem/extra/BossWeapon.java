...
public class BossWeapon extends ItemExecutor {

	private static final Random _random = new Random();

	private static final Log _log = LogFactory.getLog(BossWeapon.class);
	...
	public void execute(final int[] data, final L1PcInstance pc,
	    ...
	    L1BossWeapon bossWeapon = 
	            ExtraBossWeaponTable.getInstance().get(item.getItemId(), 1);
	    ...
	    if (power == null) {
	        ...
	        if ((_random.nextInt(1000) >= bossWeapon.getSuccessRandom())) {
	          pc.sendPackets(new S_SystemMessage(bossWeapon
	            .getFailureMsg()));
	          return;
	        }
	      ...
	      power.set_boss_weapon(bossWeapon);
			...
			final L1BossWeapon srcBossWeaopn = ExtraBossWeaponTable
					.getInstance().get(item.getItemId(), power.get_boss_lv());
			...
			if (power.get_boss_weapon() == srcBossWeaopn) {
				...
				if (_random.nextInt(1000) >= nextBossWeaopn
						.getSuccessRandom()) {

					final L1BossWeapon beginBossWeaopn = ExtraBossWeaponTable
							.getInstance().get(item.getItemId(), 1);
					final L1BossWeapon backBossWeaopn = ExtraBossWeaponTable
							.getInstance().get(item.getItemId(), power.get_boss_lv()-1);

					pc.sendPackets(new S_SystemMessage(nextBossWeaopn
							.getFailureMsg()));

					if (beginBossWeaopn == null) {
						_log.error("------ 頭目靈魂武器已經是最原始，無法歸零 ------");
						return;
					}
					
					if (backBossWeaopn == null) {
						_log.error("------ 頭目靈魂武器已經是最原始，無法歸零 ------");
						return;
					}
					

					if (ConfigAlt.DE_BOSS) {
					if(power.get_boss_lv()>1){
					    power.set_boss_lv(power.get_boss_lv()-1);
                        
						power.set_boss_weapon(backBossWeaopn);

						pc.sendPackets(new S_ItemStatus(tgItem));

						CharItemPowerReading.get().updateItem(
								tgItem.getId(), power);
					} else {
						power.set_boss_lv(1);
                        
						power.set_boss_weapon(beginBossWeaopn);
				...
				if (ConfigAlt.Cross_BOSS) {
					...
					pc.getInventory().removeItem(item, 1);
						...
						if (_random.nextInt(1000) >= newBossWeapon
								.getSuccessRandom()) {
