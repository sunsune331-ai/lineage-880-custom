...
public class Wand_Short extends ItemExecutor {
	private static final Log _log = LogFactory.getLog(Wand_Short.class);

	public static ItemExecutor get() {
		return new Wand_Short();
	}
	...
	private void set_use_type(int use_type) {
		try {
			...
			if (use_type > 0)
				_log.error("Wand_Short 可執行職業設定錯誤:餘數大於0");
		} catch (Exception e) {
			_log.error(e.getLocalizedMessage(), e);
		}
	}
