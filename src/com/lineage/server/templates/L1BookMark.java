...
public class L1BookMark {
    ...
    public static void bookmarkDB(L1PcInstance pc) {
        ...
        try {
            ...
            // _log.log(Level.WARNING, "bookmarks 例外發生。", e);
        } finally {
    ...
    public static synchronized void addBookmark(L1PcInstance pc, String s) {
        ...
        if (pc.getBookMark(s) == null) {
            ...
            pc.sendPackets(new S_ServerMessage(1655));// 同記憶人保存。
        }
    }
    ...
    public static void WriteBookmark(L1PcInstance pc) {
        ...
        try {
            ...
            // _log.log(Level.WARNING, "WriteBookmark 例外發生。", e);
        } finally {
            SQLUtil.close(rs, pstm, con);
        }
    }
    ...
    public synchronized static void Bookmarkitem(L1PcInstance pc, L1ItemInstance useItem, int obj_id, boolean isChange) {
        ...
        try {
            ...
            rs.next();
            int Numid = rs.getInt("newid");
            pstm = con.prepareStatement("SELECT * FROM extra_book_mark WHERE item_obj_id='" + obj_id + "' ORDER BY num_id ASC");
            rs = pstm.executeQuery();
            if (pc.getMark_count() == size) {
                //pc.sendPackets(new S_SystemMessage("記憶桁一。[擴張必要]"));
                pc.sendPackets(new S_SystemMessage("達到記憶座標數量上限,無法再記錄座標。"));
                return;
            }
            while (rs.next()) {
            ...
            //pc.sendPackets(new S_SystemMessage("[ " + count2 + " ]記憶登錄"));
            pc.sendPackets(new S_SystemMessage("已保存[ " + count2 + " ]個記憶座標,請重登入一次進行更新。"));
            if (count != 0) {
                //pc.sendPackets(new S_SystemMessage("[ " + count + " ]記憶保存。[擴張必要]"));
                pc.sendPackets(new S_SystemMessage("[ " + count + " ]個記憶保存失敗，[需要擴充]。"));
            }
