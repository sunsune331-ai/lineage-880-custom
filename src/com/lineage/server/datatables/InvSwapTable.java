...
import com.lineage.server.serverpackets.S_InventorySwap;
 ...
 * 裝備切換
 */
public final class InvSwapTable {
    private static InvSwapTable _instance;
    private static Map<Integer, Map<Integer, List<Integer>>> list;
    private static Map<Integer, Integer> code_list; // 當前設定位置。

    public static InvSwapTable getInstance() {
        if (_instance == null) {
            _instance = new InvSwapTable();
        }
        return _instance;
    }
    ...
    public void toSaveSet(L1PcInstance pc, int code) {
        ...
        synchronized (set) {
            ...
            if (db.size() > 21) {
                System.out.println("穿的數量總計超過" + pc.getName());
            }
        }
    }
}
