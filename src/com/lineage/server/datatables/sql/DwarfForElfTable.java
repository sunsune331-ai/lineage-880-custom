...
public class DwarfForElfTable implements DwarfForElfStorage {
	...
	public void load() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM `character_elf_warehouse` order by item_id");
	...
	public CopyOnWriteArrayList<L1ItemInstance> loadItems(String account_name) {
		CopyOnWriteArrayList<L1ItemInstance> list = (CopyOnWriteArrayList<L1ItemInstance>) _itemList.get(account_name);
		if (list != null) {
			return list;
		}
		return null;
	...
	public void insertItem(String account_name, L1ItemInstance item) {
		_log.warn("帳號:" + account_name + " 加入妖精倉庫數據:" + item.getNumberedName(item.getCount(), false) + " OBJID:"
				+ item.getId());
		addItem(account_name, item);
	...
	public void updateItem(L1ItemInstance item) {
		_log.warn(" 更新妖精倉庫數據:" + item.getNumberedName(item.getCount(), false) + " OBJID:" + item.getId());
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = DatabaseFactory.get().getConnection();
	...
	public void deleteItem(String account_name, L1ItemInstance item) {
		CopyOnWriteArrayList<L1ItemInstance> list = _itemList.get(account_name);
		if (list != null) {
			_log.warn("帳號:" + account_name + " 妖精倉庫物品移出:" + item.getNumberedName(item.getCount(), false) + " OBJID:"
					+ item.getId());
			list.remove(item);
 ...
 * com.lineage.server.datatables.sql.DwarfForElfTable JD-Core Version: 0.6.2
 */