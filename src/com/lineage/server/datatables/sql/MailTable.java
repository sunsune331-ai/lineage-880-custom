...
public class MailTable implements MailStorage {
	...
	public void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				final int isReMail = rs.getInt("Is_ReMail");
				mail.setReMail(isReMail != 0 ? true : false);
				// 檢查名稱是否以被使用
	...
	@Override
	public void writeMail(L1Mail mail, final int type, final String receiver, final L1PcInstance writer,
			final byte[] text, final int isReMail) {
		...
		try {
			...
			pstm2 = con.prepareStatement("INSERT INTO `character_mail` SET "
					...
					+ "`date`=?,`read_status`=?,`subject`=?,`content`=?,`Is_ReMail`=?");
