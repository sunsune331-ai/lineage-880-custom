...
public class pcMove extends NpcMoveExecutor {
	...
	public void setDirectionMove(final int dir) {
		if (_pc.hasSkillEffect(L1SkillId.STATUS_FREEZE)) { //被凍時不能移動 QQ:759347094
			return;
		}
		if (dir >= 0) {
			int locx = _pc.getX();
	...
	public int moveDirection(int x, int y) {
		...
		try {
			...
			if (_pc.hasSkillEffect(L1SkillId.STATUS_FREEZE) && d >= -1D) {
				return -1;
			}
			if (_pc.hasSkillEffect(L1SkillId.STATUS_CURSE_PARALYZED) && d >= -1D) {
				return -1;
			}
			if (_pc.hasSkillEffect(L1SkillId.STATUS_POISON_PARALYZED) && d >= -1D) {
				return -1;
			}
		...
		try {
			...
			if (_aie <= _hce) {
				if (d <= L1NpcInstance.DISTANCE) {
					_list = null;
					final CheckPathpc maze = new CheckPathpc(x, y, L1NpcInstance.DISTANCE, _pc);
					_cxy = maze.cxy();
	...
	private boolean glanceCheck(final int tx, final int ty, final int th) {
		...
		for (int i = 0; i < 15; i++) {
			if (((chx == tx) && (chy == ty)) || ((chx + 1 == tx) && (chy - 1 == ty))
				...
				} else if (map.isExistDoor(chx, chy) == true) {
					return false;
				}
				if (chx < tx) {
					if (chy == ty) {
