...
public class W_SK0011 extends L1WeaponSkillType {
	private static final Log _log = LogFactory.getLog(W_SK0011.class);
	...
	public double start_weapon_skill(L1PcInstance pc, L1Character target, L1ItemInstance weapon, double srcdmg) {
		try {
			...
			if (random(weapon) >= chance) {
				...
				double hpadd = calc_dmg(pc, target, outdmg, weapon);
				
				if ((target instanceof L1NpcInstance)) {
					L1NpcInstance tgNpc = (L1NpcInstance) target;
					if(tgNpc.getNpcId()>=45001 && tgNpc.getNpcId()<=45004){
						pc.sendPackets(new S_ServerMessage("冰裂傷害值:"+calc_dmg(pc, target, outdmg, weapon)));
						pc.sendPackets(new S_ServerMessage("吸血量:"+hpadd));
						hpadd=0;
					}
					
				}
                if(hpadd>0){
                	int newHp = (int) (pc.getCurrentHp() + hpadd);
