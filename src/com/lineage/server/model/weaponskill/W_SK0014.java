...
public class W_SK0014 extends L1WeaponSkillType {
	...
	public double start_weapon_skill(L1PcInstance pc, L1Character target, L1ItemInstance weapon, double srcdmg) {
		try {
			...
			int chance = _random.nextInt(1200);
			if (random(weapon) >= chance) {
				...
				double damage = 0;//= DmgAcMr.getDamage(pc);
				/*
				 * if ((target.getCurrentHp() - damage < 0) &&
				 * (target.getCurrentHp() != 1)) { damage =
				 * target.getCurrentHp() - 1; } else if (target.getCurrentHp()
				 * == 1) { damage = 0.0D; }
				 */

				double SPINT = DmgAcMr.getSpInt(pc);
				int dice = _type1 + _random.nextInt(_type2 - _type1);
				int def = calc_resist(target);

				damage = calc_mr(pc, target, (((SPINT - 4 - def) * dice / 32) * bon()), weapon);

				// Int [ Int [ Int [ 亂數 * { 3*(智力+道具sp) - 4 - 屬抗參數 } / 32 ] * (3/2)(爆擊) ] * 抗魔係數 ] - 減傷
