...
public class L1Search implements L1CommandExecutor {
		...
		try {
			...
			while (st.hasMoreTokens()) {
				...
				if (tempVar == null || tempVar == "") {
					pc.sendPackets(new S_SystemMessage("請輸入要找的字串"));
					return;
				}
				if (itCount == 0 && (tempVar.equals("防具") || tempVar.equals("武器") || tempVar.equals("道具")
						|| tempVar.equals("變身") || tempVar.equals("NPC"))) {
					simpleS = false;
					type = tempVar;
				} else {
	...
	private void find_object(L1PcInstance pc, String type, String name) {
		try {
			...
			if (type.equals("防具")) {
				statement = con
						.prepareStatement("SELECT item_id,name,bless FROM armor WHERE name Like '%" + name + "%'");
			} else if (type.equals("武器")) {
				statement = con
						.prepareStatement("SELECT item_id,name,bless FROM weapon WHERE name Like '%" + name + "%'");
			} else if (type.equals("道具")) {
				statement = con
						.prepareStatement("SELECT item_id,name,bless FROM etcitem WHERE name Like '%" + name + "%'");
