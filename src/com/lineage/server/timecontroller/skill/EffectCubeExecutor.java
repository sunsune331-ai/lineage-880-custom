...
public class EffectCubeExecutor {
	...
	private static void cubeToAlly(final L1Character cha, final L1EffectInstance effect) {
		...
		if (!cha.hasSkillEffect(skillid)) {
			...
			case isCubeBurn:
                if (cha.hasSkillEffect(204)) {
                	cha.removeSkillEffect(204);
                }
				cha.addDmgup(4);
				cha.addHitup(4);
				break;
			case isCubeEruption:
                if (cha.hasSkillEffect(214)) {
                	cha.removeSkillEffect(214);
                }
				cha.addAc(-8);
				break;
			case isCubeShock:
                if (cha.hasSkillEffect(209)) {
                	cha.removeSkillEffect(209);
                }
				cha.addSp(2);				
				break;
			case isCubeHarmonize:
                if (cha.hasSkillEffect(219)) {
                	cha.removeSkillEffect(219);
                }
