...
public class DIYGfxTimer extends TimerTask {
	...
	private static final Log _log = LogFactory.getLog(DIYGfxTimer.class);
	...
	public void run() {
		try {
			...
			for (final L1PcInstance tgpc : _DIYList) {
				if (check(tgpc)) {
					int add_gif = tgpc.get_DIY_gfx();
					int add_gif_time = tgpc.get_DIY_time();
			...
			// _log.error("DIY定時特效處理時間軸異常重啟", e);
			PcOtherThreadPool.get().cancel(_timer, false);
			final DIYGfxTimer expTimer = new DIYGfxTimer();
			expTimer.start();
		}
	}
